...
def signin():
	...
	if Form.validate_on_submit():
		...
		if user is not None and user.verify_password(Form.Password.data):
			login_user(user, remember_me.data) # Records user as logged in, in a session
			return redirect(url_for('main.user_account'))
		# If the email exists, password is checked to see if it matches the one in the 
		# db. If it does, the user is logged in and proceeds to 
		# url_for(main.user_account). 
		flash('Invalid Business/Organization Email or Password.')
		# Else, the message below is flashed.
